...
class window1(tk.Frame):
    ...
    def __init__(self):
        ...
        self.lb_username = tk.Label(self, text="USERNAME : ", fg="white",bg="blue")
        self.lb_username.place(x=150, y=200)
        self.tv_username = tk.Entry(self, width=100)
        self.tv_username.place(x=300, y=200)

        self.lb_pass = tk.Label(self, text="PASSWORD : ",fg="white", bg="blue")
        self.lb_pass.place(x=150 , y=300 )
        self.tv_pass = tk.Entry(self, show="*", width=100)
        self.tv_pass.place(x=300, y=300 )
        self.btnLogin = Button(self, text = 'Login', background = "blue", fg = "white",width =17,command=self.checkLogin)
...
class window2(tk.Frame):
    ...
    def __init__(self):
        ...
        lbl = tk.Label(new, text="Enter Enrollment", width=20, height=2, fg="white", bg="deep pink", font=('times', 15, ' bold '))
        ...
        new.clearButton = tk.Button(new, text="Clear",fg="black"  ,bg="deep pink"  ,width=10  ,height=1 ,activebackground = "Red" ,font=('times', 15, ' bold '))
        new.clearButton.place(x=800, y=210)

        new.clearButton1 = tk.Button(new, text="Clear",fg="black"  ,bg="deep pink"  ,width=10 ,height=1, activebackground = "Red" ,font=('times', 15, ' bold '))
        new.clearButton1.place(x=800, y=310)

        AP = tk.Button(new, text="Check Register students",fg="black" ,command=admin_panel ,bg="cyan"  ,width=19 ,height=1 ,font=('times', 15, ' bold '))
...
def subjectchoose():
                ...
                while True:
                    ...
                    if process_this_frame:
                        ...
                        if len(face_encodings) > 0:
                            ...
                            #predictions = [(le.inverse_transform(int(pred)).title(), loc) if rec for pred, loc, rec in 
                                        #zip(clf.predict(face_encodings), face_locations, is_recognized)]
                                #predictions = [(le.inverse_transform(int(pred)).title(), loc) if rec else ("Unknown", loc) for pred, loc, rec in
                                           #zip(clf.predict(face_encodings), face_locations, is_recognized)]
                    ...
                    for name, (top, right, bottom, left) in predictions:
                            ...
                            #predictions = [("Unknown", loc)if not rec for pred, loc, rec in zip(clf.predict(face_encodings), face_locations, is_recognized)]
                    #process_this_frame = not process_this_frame
    ...
    def Attf():
        import subprocess
        subprocess.Popen(r'explorer /select,"D:\project\dev\Attendace managemnt system\Attendance\-------Check atttendance-------"')
    ...
    def log_in():
        ...
        if username == 'admin' :
            if password == 'admin1234':
                ...
                cs = 'D:/project/dev/Attendace managemnt system/StudentDetails/StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
